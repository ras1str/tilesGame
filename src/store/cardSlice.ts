...
export const cardSlice = createSlice({
  ...
  reducers: {
    ...
    flipCard: (state, action) => {
      const cardIndex = state.card.findIndex((card) => card.id === action.payload.id);
      const openedIndex = state.opensCards.findIndex((card) => card.id === action.payload.id);
    ...
    check: (state) => {
      if (state.opensCards.length === 2) {
        const firsOpenCard = state.card.findIndex((post) => post.id === state.opensCards[0].id);
        const secondOpenCard = state.card.findIndex((post) => post.id === state.opensCards[1].id);
